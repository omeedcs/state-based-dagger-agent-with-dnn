...
class Team:
    ...
    def __init__(self):
        ...
        self.model = torch.jit.load(path.join(path.dirname(path.abspath(__file__)), 'goated_agent.pt'))
        self.oldPos = defaultdict(lambda: -1)
        self.oldAcc = defaultdict(lambda: -1)
